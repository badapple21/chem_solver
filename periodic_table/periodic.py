...
def lookup_element(q):
    ...
    cursor.execute(("SELECT Atomic_number FROM elements WHERE Symbol LIKE ? LIMIT 3;"), (q,))
    
    #adds to list
    rtn.append(cursor.fetchall())

    #does the same but for name
    cursor.execute(("SELECT Atomic_number FROM elements WHERE Name LIKE ? LIMIT 3"), (q,))
